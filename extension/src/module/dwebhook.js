...
function validateMessage(message, expectedSolver, expectedChallengeId) {
  try {
    ...
    const requiredFields = ['title', 'description', 'color', 'url', 'timestamp', 'thumbnail'];
    ...
    if (!urlMatch || parseInt(urlMatch[1], 10) !== expectedChallengeId) {
      return {success: false, error: `Challenge ID가 일치하지 않습니다. (기대: ${expectedChallengeId}, 실제: ${urlMatch?.[1] || 'Unknown'})`};
    }

    const solverMatch = embed.description.match(/https:\/\/dreamhack\.io\/users\/(\d+)/);
    if (!solverMatch) {
    ...
    if (actualSolver !== expectedSolver) {
      return {success: false, error: `Solver ID가 저장된 userid와 일치하지 않습니다. (저장된 userid: ${expectedSolver}, 응답의 solver: ${actualSolver})`};
    }
